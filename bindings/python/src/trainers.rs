    ...
    pub(crate) fn get_as_subtype(&self, py: Python<'_>) -> PyResult<PyObject> {
        ...
        Ok(match *self.trainer.as_ref().read().unwrap() {
            ...
            TrainerWrapper::BacktrackingBpeTrainer(_) => {
                Py::new(py, (PyBacktrackingBpeTrainer {}, base))?.into_py(py)
            }
        })
    }
}
...
///         highly repetitive tokens like `======` for wikipedia
///
#[pyclass(extends=PyTrainer, module = "tokenizers.trainers", name = "BacktrackingBpeTrainer")]
pub struct PyBacktrackingBpeTrainer {}
#[pymethods]
impl PyBacktrackingBpeTrainer {
    #[getter]
    fn get_vocab_size(self_: PyRef<Self>) -> usize {
        getter!(self_, BpeTrainer, vocab_size)
    }
    ...
    #[pyo3(signature = (**kwargs), text_signature = None)]
    pub fn new(kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<(Self, PyTrainer)> {
        let mut builder = tk::models::backtracking_bpe::BacktrackingBpeTrainer::builder();
        if let Some(kwargs) = kwargs {
            for (key, val) in kwargs {
                let key: String = key.extract()?;
                match key.as_ref() {
        ...
        Ok((PyBacktrackingBpeTrainer {}, builder.build().into()))
    }
}
