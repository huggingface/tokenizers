//! The `PipelineTokenizer`'s **parallel** encode must produce exactly the same
//! token ids as its serial encode and as the reference `Tokenizer` (the oracle).
//! `pipeline_oracle.rs` covers per-model id equality; this file covers the job
//! runtime: chunking, batching, striding and the two `EncodeHandle` consumers. Exercised over the bert-wiki
...
//! The post-processor is applied (bert-wiki frames with `[CLS]`/`[SEP]`), so the
//! oracle is called with `add_special_tokens = true` to match `encode`'s default --
//! which also checks that the framing lands once per *input*, not per chunk of a
//! strided one.
...
fn check_chunks(corpus: &str, target_bytes: usize) {
    ...
    for chunk in make_chunks(&text, target_bytes) {
        let expected = oracle.encode(chunk.as_str(), true).unwrap();
...
fn check_small_batch(corpus: &str) {
    ...
    for (input, tokens) in inputs.iter().zip(&got) {
        let expected = oracle.encode(*input, true).unwrap();
...
fn check_intra_seq(corpus: &str) {
    ...
    let got: Vec<u32> = pipeline
        ...
        .map(|t| t.id)
        .collect();
    let expected = oracle.encode(doc.as_str(), true).unwrap();
    assert_eq!(
        expected.get_ids(),
        got.as_slice(),
