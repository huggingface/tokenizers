...
/// Inputs built to break word boundaries, which the natural corpora do not
/// reach. `<b> </b>` is the one that matters most: gemma-4 merges its `>`, mark
/// and `</` into a single token across what looks like a word boundary.
const HOSTILE: &[&str] = &[
