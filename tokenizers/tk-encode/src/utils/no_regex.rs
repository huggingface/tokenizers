...
//! *regex* pattern ever asks for it: the bitcanon-native pre-tokenizers (GPT-2, cl100k, deepseek, the
//! class family, char-delimiter) need no backend, and a plain string pattern is searched for directly
//! (`bitcanon::literal`). A regex bitcanon does not cover errors at load time with a clear message.
