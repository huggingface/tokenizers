...
// Optional system-regex backend, needed only for a *regex* pattern that bitcanon does not cover.
// With `fancy-regex` off a stub compiles and those patterns error at load. Everything else works
// regardless: the bitcanon-native pre-tokenizers, and any `Split` or `Replace` whose pattern is a
// plain string (searched for directly, see `bitcanon::literal`).
...
// Recognize known GPT pre-tokenization regexes and route them to bitcanon's native (unrolled) FSM.
mod unrolled_regex;
