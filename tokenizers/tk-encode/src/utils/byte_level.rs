...
// under the historical name so call sites are unchanged. `pub` because it has been public API since
// before the split: the `ByteLevel` pre-tokenizer lowering — which rewrites a `use_regex` ByteLevel
// into a `Split` on exactly this pattern — is `tk-serialize`'s now, and reaches
// `atomsplit::regexes::GPT2` directly.
