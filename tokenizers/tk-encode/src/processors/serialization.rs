...
//!
//! The one processor whose tag *is* required is `Sequence`, and it is not here: it is `tk-convert`'s
//! own type, built by `impl_serde_type!`. It has to be strict, because `{"processors":[…]}` with no
//! tag would otherwise be indistinguishable from nothing at all.
