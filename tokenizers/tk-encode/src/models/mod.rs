...
// `ModelWrapper` and its hand-written `Deserialize` are `tk-convert`'s, and so is the config-shaped
// `BPE`. What is left in this tree is the engines that actually encode -- `PipelineBPE` is the only
// BPE among them -- plus, below, the one serde helper they share.
...
/// diff nobody wants to read. `pub` because `tk-convert` writes a `vocab.json` for `BPE` and
/// `WordLevel` through it as well.
#[cfg(feature = "serde")]
