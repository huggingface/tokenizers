...
enum Search {
    ...
    Regex(SysRegex),
    /// The empty string, which matches nothing. `Pattern for &str` makes the same choice, and a
    /// pattern that matched everywhere would replace the whole text with copies of `content`.
    Nothing,
}
