...
mod tests {
    ...
    /// What the Bert normalizer hands the pre-tokenizer: every CJK char padded with a
    /// space on each side.
    fn space_padded_cjk(text: &str) -> String {
        text.chars()
            .flat_map(|c| {
    ...
    #[test]
    fn chinese_chars() {
        let padded = space_padded_cjk("野口里佳 Noguchi Rika");
        assert_eq!(
            pretokenize(&padded),
            vec![
                ("野", (1, 4)),
