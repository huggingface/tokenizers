...
// SAFETY: the spans come from `bitcanon::classes::CharDelimiterSplit`, which splits only at character
...
unsafe impl pipeline::PreTokenizer for CharDelimiterSplit {
    fn pre_tokenize(
        ...
        // native bitcanon FSM (memchr-backed single-byte scan); `Removed` — drops the delimiter,
        ...
        scratch.split_on_bytes(
            text.as_bytes(),
            |bytes, spans| {
                bitcanon::classes::CharDelimiterSplit(self.delimiter).pre_tokenize(
                    bytes,
                    &mut [],
                    spans,
                )
