...
impl PipelineTokenizer {
    ...
    fn plan(&self, sample: &str) -> ParallelPlan {
        if let Some(boundary) = self.stride_boundary() {
            if cut_exists(sample, boundary) {
                // `Raw`: no serial prefix, so always worth it when the input can be cut.
                return ParallelPlan::Raw(boundary);
            }
        }
        if matches!(self.inner.model, PipelineModel::WordLevel(_)) {
            ...
            return ParallelPlan::Whole;
        }
        if let Some(boundary) = self.normalized_stride_boundary() {
            ...
            if cut_exists(sample, boundary) {
                return ParallelPlan::Normalized(boundary);
            }
        }
        // No usable cut: pre-tokenize serially and parallelise the model over span groups.
        // The serial prefix goes through the normal pre-tokenizer, so it is `bitsplit`
        // wherever `bitsplit` is wired (gpt2, cl100k) -- which is what makes this fallback
        // cheap enough to prefer over a degenerate stride.
