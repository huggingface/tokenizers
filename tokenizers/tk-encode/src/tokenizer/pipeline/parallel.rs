...
mod tests {
    ...
    #[test]
    fn the_partition_puts_whole_task_chunks_first_and_keeps_them_all() {
        let lens = [
            1usize,
            PARALLEL_MIN_BYTES,
            3,
            PARALLEL_MIN_BYTES * 9,
            0,
            PARALLEL_MIN_BYTES - 1,
            PARALLEL_MIN_BYTES + 1,
            700,
        ];
        let mut chunks: Vec<SequenceChunk> = lens
            .iter()
            .map(|&len| SequenceChunk {
                seq: 0,
                idx: 0,
                side: Seq::A,
                range: 0..len,
        ...
        assert_eq!(boundary, 3, "three chunks are a task on their own");
        assert!(
            chunks[..boundary]
                .iter()
                .all(|c| c.range.len() >= PARALLEL_MIN_BYTES),
            "a chunk below the threshold got into the front"
        );
        assert!(
            chunks[boundary..]
                .iter()
                .all(|c| c.range.len() < PARALLEL_MIN_BYTES),
            "a whole-task chunk got left in the tail"
        );

        let mut got: Vec<usize> = chunks.iter().map(|c| c.range.len()).collect();
        let mut want = lens.to_vec();
        got.sort_unstable();
        want.sort_unstable();
        assert_eq!(got, want, "the partition lost or invented a chunk");
    }
}
