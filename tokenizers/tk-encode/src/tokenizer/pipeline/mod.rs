...
// The differential "parallel == serial" tests were deleted with `Tokenizer` (they needed one to
// build from); an input has to be sized past the threshold to reach the parallel path at all.
#[cfg(feature = "parallelism")]
...
// `pub` because [`NormalizerChain`] below implements `pipeline::Normalizer` by running exactly
// this, and anything holding its own slice of normalizers wants the same loop rather than a copy.
...
// `pub` because `tk-serialize` builds these straight from the config it reads.
#[derive(Debug)]
...
/// the `Sequence` normalizer the reader flattened away did.
// `pub` for the same reason as `PipelineNormalizer`: `tk-serialize` needs it to replay added tokens
// through a lowered chain. `add_tokens` takes one `Normalizer`, and the chain is a slice -- handing
// it the first member alone would normalize an added token differently from the text.
...
impl PipelinePostProcessor {
    ...
    ///
    /// Exists so the reader stops at a door rather than at the fields: `tk-serialize` builds the
    /// two templates from JSON and hands them over, and nothing outside needs the fields
    /// themselves.
...
// `Slice`, `Seq`, `Template` and `compose` below are `pub` because `tk-serialize` lowers a config's
// post-processor in terms of them. `compose`'s "at most one arranging member" rule is the part here
// that decides ids, so it stays single-sourced rather than being restated on the reader's side.
#[derive(Clone, Debug)]
pub enum Slice {
    Specials {
...
impl PipelineTokenizer {
    ...
    /// the scratch pool are derived. `tk-serialize`'s reader goes through it, and so would anything
    /// else that ever builds a pipeline: two copies of this is how two paths drift.
    ///
