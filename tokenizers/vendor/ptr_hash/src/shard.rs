...
/// Minimal stand-in for `tempfile::TempDir`, used only by the on-disk sharding path (never
/// entered by the in-memory `Sharding::None` / `default_fast` build). Creates a uniquely-named
/// directory under the system temp dir and removes it on drop. Avoids the `tempfile` dependency.
struct TempDir {
...
impl TempDir {
    fn new() -> std::io::Result<Self> {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        path.push(format!("ptrhash-shard-{}-{}", std::process::id(), nanos));
