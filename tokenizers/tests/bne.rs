...
fn test_load_bne_from_file() {
    let _model = BNE::from_file("data/bne-vocab.json", "data/bne-merges.txt")
        .build()
        .unwrap();
}
...
fn test_decoding_with_added_bne() {
    ...
    let _ = tokenizer.add_tokens([AddedToken::from("д", false).normalized(true)]);
    let encoded = tokenizer
...
fn test_bne_tokenizer_from_file_2() {
    ...
    let _encoded = tokenizer.encode(string, false).unwrap();
