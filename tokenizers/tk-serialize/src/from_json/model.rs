...
///
/// ## The scores match `serde_json` bit for bit, and are deliberately not correctly rounded
///
/// [`crate::json`] does not parse numbers with `f64::from_str`, which is correctly rounded. It
/// reproduces what `serde_json` does without its `float_roundtrip` feature: accumulate the digits
/// into a `u64` and divide by a power of ten, which is off by one ULP for 8334 of t5's 32100
/// scores. Every score there is exactly an `f32` widened to `f64`, and `from_str` lands on it; this
/// reader lands next to it, on purpose.
///
/// The lattice is a sum of scores, so one ULP only matters where two segmentations very nearly tie —
/// 2 ids out of 1.25 M on the english fixture, always a run of one repeated character. Being more
/// accurate would move those ids, and the ids that ship today are the contract: turning on
/// `serde_json/float_roundtrip` was measured to change them. So bug-compatibility is the
/// requirement, and reproducing the arithmetic is the design rather than something to paper over.
#[cfg(feature = "unigram")]
