...
/// plus a `Split`. This is the only place that decomposition happens now -- the config path's copy
/// went with the config layer -- so the three settings refused below are the whole rule.
///
