...
//!
//! This is the slim read path. It parses the config with [`crate::json`] and constructs the
//! pipeline-native types directly, because naming a wrapper enum makes every variant of it
//! reachable, and that reachability is most of what this exercise exists to remove. In particular
//! neither `ModelWrapper` nor `NormalizerWrapper` is ever named — both are gone outright now,
//! deleted with the config layer: the concrete model goes straight into a [`PipelineModel`], and
//! each normalizer into its own [`PipelineNormalizer`] variant.
//!
...
//!
//! `Slice` and `Template` are `tk-encode`'s own types rather than copies made here, so the
//! post-processor is lowered by exactly the code that decides ids rather than by a second copy
//! of the rules. The pipeline itself is assembled through [`PipelineTokenizer::from_parts`].
//!
...
//! assignment the released library produces. Getting this wrong drifts ids silently, and the gate
//! that catches it is `tk-convert`'s `pipeline_oracle`, which compares ids against the released
//! crate.
//!
...
//! lowers it. That is what let `ModelWrapper` go — the one type whose mere mention dragged in every
//! model's deserializer.
//!
...
fn unsupported(what: &str) -> tk_encode::Error {
    format!(
        "the slim JSON reader does not support {what}. Convert the config offline with \
         `tk_convert::canonicalize_file`."
    )
    .into()
}
...
fn from_json_value(doc: &Json<'_>) -> Result<PipelineTokenizer> {
    ...
    match kind {
        ...
        // config-shaped `BPE` that used to play that part was deleted with the config layer, and
        // `PipelineBPE` needs the merges resolved against the vocabulary. `build` gets the
        // vocabulary instead -- see [`VocabOnly`] -- and lowers it into the model afterwards, which
        // keeps the replay-then-lower order intact.
        "BPE" => {
...
fn model_kind(cfg: &Json<'_>) -> &'static str {
    ...
    // machinery's ~480. The order is the one the deleted `ModelUntagged` tried its variants in,
    // because that is what decided ties for every config written until now: BPE first, then
    // WordPiece before WordLevel (WordLevel's shape is a subset).
