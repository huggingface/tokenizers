...
mod tests {
    ...
    /// The accessors, on the shape they exist for. `hifijson` owns the scanning, the string
    /// escapes and the `\u` handling, so what is ours is [`JsonExt`]: the lookups, the `null`-is-
    /// absent rule, and the checked integer reads.
    #[test]
    fn the_accessors_read_a_tokenizer_json_shaped_document() {
        // Checked reads, which is where an accessor refuses rather than returns: an id is a `u32`,
        // and everything that is not one has to read as absent instead of being truncated into one.
        ...
        // A `dropout: null` field reads as absent, which is how the BC default works: present via
        // `get`, absent via `get_some`.
        assert!(model.get("dropout").is_some());
