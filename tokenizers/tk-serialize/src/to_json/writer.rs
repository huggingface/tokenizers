...
pub(super) struct Out {
    ...
    after_key: bool,
    /// `Some(depth)` while a subtree is being written without any whitespace, where `depth` is the
    /// nesting level that asked for it. Set for the bulk collections only -- a vocabulary, a merge
    /// list, the added tokens -- because indenting those is what makes a `tokenizer.json` enormous:
    /// `gemma-4.json` is 32.2 MB pretty-printed and 13.6 MB without the whitespace, so 18.6 MB of
    /// it is indentation. The skeleton stays indented, which is the part anyone reads.
    compact_from: Option<usize>,
}
...
impl Out {
    ...
    /// Stop indenting until the matching [`Self::end_compact`]. Nests: an inner call is a no-op, so
    /// the merge list's inner pairs stay on the one line their enclosing array opened.
    pub(super) fn begin_compact(&mut self) {
        if self.compact_from.is_none() {
            self.compact_from = Some(self.empty.len());
        }
    }

    /// Resume indenting, if this is the depth that suspended it.
    pub(super) fn end_compact(&mut self) {
        if self.compact_from == Some(self.empty.len()) {
            self.compact_from = None;
        }
    }

    fn indenting(&self) -> bool {
        self.compact_from.is_none()
    }

    /// A newline and two spaces per open container.
    fn break_line(&mut self) {
        self.buf.push('\n');
        for _ in 1..self.empty.len() {
            self.buf.push_str("  ");
        }
    }
    ...
    fn sep(&mut self) {
        ...
        // Before every member including the first, so a container reads as `{\n  "a": 1\n}`. Not at
        // the document root, which has no enclosing container to be indented inside.
        if self.indenting() && self.empty.len() > 1 {
            self.break_line();
        }
    }
    ...
    fn close(&mut self, brace: char) {
        // An empty container closes as `{}` or `[]`; anything else puts its brace on its own line,
        // indented to match the line that opened it.
        let was_empty = self.empty.pop().expect("a container is open");
        if self.indenting() && !was_empty {
            self.break_line();
        }
        self.buf.push(brace);
    }
...
mod tests {
    ...
    fn floats_round_trip_through_our_own_parser() {
        ...
        // And the values with no JSON form at all are refused rather than mangled into one.
    ...
    /// and non-ASCII goes out raw, which is what keeps a byte-level vocabulary readable.
    #[test]
