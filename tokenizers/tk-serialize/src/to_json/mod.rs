...
pub fn to_json(tokenizer: &PipelineTokenizer) -> Result<String> {
    ...
    // Key order deviates from what `Tokenizer::save` wrote: `added_tokens` moves to the end, next to
    // the model, so both bulk sections are last and everything a human reads comes first. The reader
    // looks keys up rather than reading them in order, so nothing depends on this.
    out.obj_open();
