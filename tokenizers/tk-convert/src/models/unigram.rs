...
//!
//! Both are serde over the whole model, so neither could stay with the model in `tk-encode`. The
//! shape is [`mirror::unigram`]'s; these two just point it at a file.
...
use super::mirror::{UnigramOwned, UnigramRef};
...
pub fn load<P: AsRef<Path>>(path: P) -> Result<Unigram> {
    ...
    Ok(serde_json::from_str::<UnigramOwned>(&string)?.0)
}
...
pub fn save(model: &Unigram, folder: &Path, name: Option<&str>) -> Result<Vec<PathBuf>> {
    ...
    fullpath.push(name);
    let string = serde_json::to_string_pretty(&UnigramRef(model))?;
