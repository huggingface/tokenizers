//! The config-shaped `BPE`: the model as a `tokenizer.json` writes it down.
//!
//! This is the type that lets an *old* serialized BPE still be read. It is not the encode path any
//! more — [`PipelineBPE`] is, and `impl TryFrom<(BPE, bool)> for PipelineBPE` below is how one
//! becomes the other. What is left here is everything about the *config*: a builder with the ten
//! options a config can name, the vocab-and-merges loaders for the split `vocab.json` +
//! `merges.txt` spelling, the serde in `serialization`, the `Word`/merge machinery `tk-train`
//! trains into, and a `Model` impl so a `Tokenizer` built by hand still encodes.
//!
//! It lives in `tk-convert` and not in `tk-encode` because every one of those responsibilities is
//! config-layer work: `read_file` needs a JSON parser, the serde impls need serde, and the runtime
//! crate is meant to link neither.
...
use tk_encode::models::bpe::{
    DEFAULT_CACHE_CAPACITY, Error, MAX_LENGTH, MergeMap, Merges, Pair, PipelineBPE,
    PipelineBpeOptions, Vocab, VocabR,
};
...
pub use word::{FirstLastIterator, WithFirstLastIterator, Word};
...
pub fn convert_merges_to_hashmap<I: Iterator<Item = String>>(
    iter: I,
    _vocab: &Vocab,
) -> Result<Merges> {
    let mut merges = vec![];
...
impl BPE {
    ...
    pub(crate) fn merge_word(&self, w: &str) -> Result<Word> {
...
/// Lower a config-shaped [`BPE`] into the runtime [`PipelineBPE`].
///
/// `with_byte_level` is not a property of the model: it says whether the *pre-tokenizer* alongside
/// it is a `ByteLevel`, which is what decides whether the vocabulary's entries are the byte-level
/// spelling of their bytes or the bytes themselves. Only the caller that read the whole config knows
/// that, which is why it is a second argument rather than a field.
///
/// This used to be `PipelineBPE::from_bpe`, an inherent method on the other side of the crate
/// boundary. It moved here with `BPE`, because the runtime cannot name the config type — and it is a
/// free function rather than a `TryFrom`, because the orphan rule does not accept `(BPE, bool)` as a
/// local type: both the trait and `PipelineBPE` are foreign, and a tuple holding a local type is not
/// itself one.
///
/// It hands over the two tables the builder already resolved, so the derivation in
/// [`PipelineBPE::from_vocab_and_merges`] is not run a second time.
pub fn into_pipeline(model: BPE, with_byte_level: bool) -> Result<PipelineBPE> {
    let BPE {
        vocab,
        merges,
        cache,
    ...
    PipelineBPE::from_merge_map(
        ...
        PipelineBpeOptions {
            ...
            dropout,
            ignore_merges,
            // A `BPE` with no cache asked for capacity 0, which is how the options spell it too.
            cache_capacity: cache.map_or(0, |cache| cache.capacity),
            with_byte_level,
        },
    )
}
