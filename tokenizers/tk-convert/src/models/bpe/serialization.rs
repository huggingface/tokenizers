//! serde for the config-shaped [`BPE`].
//!
//! Not a mirror: `BPE` is this crate's own type now, so the impls the orphan rule used to block are
//! simply written here. The shape is unchanged, including the two backwards-compatible spellings of
