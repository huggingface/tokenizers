//! The `Sequence` post-processor. Lives here rather than in `tk-encode` because it is a
//! `Vec<PostProcessorWrapper>`, and a `Vec` of a type can only be parameterised where that type is.
...
mod tests {
    ...
    use tk_encode::{Encoding, PostProcessor};
