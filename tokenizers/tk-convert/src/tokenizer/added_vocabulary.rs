//! The legacy `AddedVocabulary`: an Aho-Corasick matcher over the added tokens, plus the
//! `added_tokens` array of a `tokenizer.json`.
//!
//! The encode path uses `tk_encode::vocab::bucket_added_vocabulary` instead; this one is what the
//! `Tokenizer` carries, and what [`crate::lowering`] replays into the bucket version.
...
use tk_encode::tokenizer::normalizer::Range;
use tk_encode::{Model, NormalizedString, Normalizer, Offsets, PreTokenizedString, Result, Token};
