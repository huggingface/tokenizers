//! The `Tokenizer` orchestration, and everything that reads or writes a `tokenizer.json`.
//!
//! Pure motion out of `tk-encode`'s `tokenizer/mod.rs`: `TokenizerBuilder`, the generic
//! `TokenizerImpl`, the concrete `Tokenizer` (a `TokenizerImpl` over the five wrapper enums),
//! `DecodeStream`, and the `from_file` / `from_str` / `from_bytes` / `from_pretrained` / `save`
//! surface. The traits these are generic over — `Model`, `Normalizer`, `PreTokenizer`,
//! `PostProcessor`, `Decoder` — stay in `tk-encode`, because that is what the components implement.
...
use tk_encode::utils::parallelism::MaybeParallelIterator;

use tk_encode::{
    Decoder, EncodeInput, Encoding, Error, InputSequence, Model, NormalizedString, Normalizer,
    OffsetReferential, OffsetType, PaddingParams, PostProcessor, PreTokenizedString, PreTokenizer,
    Result, TruncationParams, TruncationStrategy, pad_encodings, truncate_encodings,
};
...
impl Tokenizer {
    ...
    pub fn from_pretrained<S: AsRef<str>>(
        identifier: S,
        params: Option<tk_encode::utils::from_pretrained::FromPretrainedParameters>,
    ) -> Result<Self> {
        let tokenizer_file =
            tk_encode::utils::from_pretrained::from_pretrained(identifier, params)?;
...
///     .with_model(bpe)
///     .with_decoder(Some(MetaspaceDecoder::default()))
///     .with_normalizer(Some(NFC))
///     .with_pre_tokenizer(Some(ByteLevel::default()))
///     .with_post_processor(Some(ByteLevel::default()))
///     .build()
...
// Every test here builds on `test_tokenizer()`, which uses WordLevel as a trivially-predictable
// model to exercise truncation; the module is about truncation, not about WordLevel.
#[cfg(test)]
mod tests {
    use super::*;
    use tk_encode::TruncationDirection;
    use tk_encode::models::wordlevel::WordLevelBuilder;
