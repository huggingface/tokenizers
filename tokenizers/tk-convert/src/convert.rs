...
//!
//! This is the point of the crate, stated as a function. `tk-serialize` is a serde-free reader --
//! a `hifijson` parse and a builder -- for the format *as it is written today*; every shape
//! older than that is refused there with a message naming what to convert, and this module is
//! what does the converting. It fills in the fields the canonical reader expects to find and
//! rewrites the ones that changed spelling, so that the reader needs no backwards-compatibility
//! branch of its own.
//!
//! Nothing here builds a component. It reads a `serde_json::Value`, edits it, and hands it back:
//! no model, no `Metaspace`, no regex compiled. That matters because a converter that went
//! through config types would have to link every one of them, and the whole reason the
...
///
/// The order of the tests is the load-bearing part. It is the order the deleted `ModelUntagged`
/// tried its variants in -- which is what decided the kind for every config written until now --
/// and the order `model_kind` in `tk-serialize/src/from_json/mod.rs` still uses:
///
/// 1. has `merges` ⇒ BPE
