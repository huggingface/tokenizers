...
use super::{
    super::bpe::Pair, super::bpe::WithFirstLastIterator, super::bpe::Word, BacktrackingBpe,
};
...
impl BpeTrainerBuilder {
    ...
    pub fn build(self) -> BacktrackingBpeTrainer {
...
/// In charge of training a `BacktrackingBpe` model
///
/// # Examples
///
/// ```
/// use tokenizers::tokenizer::Trainer;
/// use tokenizers::models::backtracking_bpe::{BacktrackingBpe, BacktrackingBpeTrainer};
///
/// let sequences = vec![ "Hello", "World" ];
///
/// let mut trainer = BacktrackingBpeTrainer::default();
/// trainer.feed(sequences.iter(), |s| Ok(vec![s.to_owned()]));
///
/// let mut model = BacktrackingBpe::default();
/// let special_tokens = trainer.train(&mut model).unwrap();
/// ```
#[non_exhaustive]
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Eq)]
pub struct BacktrackingBpeTrainer {
...
impl Default for BacktrackingBpeTrainer {
    fn default() -> Self {
        Self::builder().build()
    }
}

impl BacktrackingBpeTrainer {
...
impl Trainer for BacktrackingBpeTrainer {
    type Model = BacktrackingBpe;

    /// Train a BacktrackingBpe model
    fn train(&self, model: &mut BacktrackingBpe) -> Result<Vec<AddedToken>> {
...
#[cfg(test)]
mod tests {
    use super::{BacktrackingBpe, BacktrackingBpeTrainer, Pair};
    ...
    fn test_train() {
        ...
        .cloned()
        .collect();
        let trainer = BacktrackingBpeTrainer::builder()
            .show_progress(false)
            .min_frequency(2)
            .build();
        let mut model = BacktrackingBpe::default();
