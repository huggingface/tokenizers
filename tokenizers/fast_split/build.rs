//! bitmap_gen — the compile-time generator for the shared classify tables (TAG_CLASSIFY_SPEC.md §1/§7).
//!
//! Runs at build time (dev/build-dep `unicode_categories`, NOT a runtime dep) and writes
//! `$OUT_DIR/atom_tables.rs`, which `src/atom_tables.rs` `include!`s. Bakes the ONE dense `Tables`
//! set (ascii / 2-byte group / 3-byte fast3 / bmp_rle / astral) read by BOTH the SIMD kernel (`vqtbl`)
//! and the scalar reader (`Tables::classify_char`). Self-validates the scalar reader reproduces the
//! reference atom for all 1.1M codepoints — a mismatch fails the build.
...
fn main() {
    ...
    // ── emit $OUT_DIR/atom_tables.rs ──
    let mut o = String::new();
    o.push_str("// GENERATED at build time by build.rs (bitmap_gen). Do not edit.\n");
    ...
    std::fs::write(format!("{out}/atom_tables.rs"), o).unwrap();
}
