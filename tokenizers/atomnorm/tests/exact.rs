...
fn exhaustive() {
    ...
    for cp in 0u32..0x30000 {
        ...
        for suffix in [
            ...
            assert_eq!(
                atomnorm::scalar::nfd(&buf),
                buf.nfd().collect::<String>(),
                "scalar NFD {cp:#x}"
            );
            assert_eq!(
                atomnorm::scalar::nfkc(&buf),
...
fn long_inputs() {
    ...
    for (k, s) in cases.iter().enumerate() {
        assert_eq!(
            atomnorm::nfd(s.as_str()),
            s.nfd().collect::<String>(),
            "NFD case {k}"
        );
        assert_eq!(
            atomnorm::scalar::nfd(s.as_str()),
            s.nfd().collect::<String>(),
            "scalar NFD case {k}"
        );
        assert_eq!(
            atomnorm::scalar::nfc(s.as_str()),
            s.nfc().collect::<String>(),
            "scalar NFC case {k}"
        );
        assert_eq!(
            atomnorm::nfkd(s.as_str()),
            s.nfkd().collect::<String>(),
            "NFKD case {k}"
        );
        assert_eq!(
            atomnorm::nfc(s.as_str()),
            s.nfc().collect::<String>(),
            "NFC case {k}"
        );
        assert_eq!(
            atomnorm::nfkc(s.as_str()),
