...
| Name | Description | Example |
| :--- | :--- | :--- |
| ByteLevel | Splits on whitespaces while remapping each byte of the input string's UTF-8 representation to a visible character. It does not consume raw byte sequences directly: if arbitrary bytes are decoded as Latin-1 first, bytes above 127 are encoded as UTF-8 and can become multiple visible characters. This technique was introduced by OpenAI with GPT-2 and has some useful properties: <ul> <li>A tokenizer using this only requires **256** characters as initial alphabet (the number of values a byte can have), as opposed to the 130,000+ Unicode characters.</li> <li>A consequence of the previous point is that it is unnecessary to have an unknown token using this since we can represent anything with 256 tokens.</li> <li>For non ascii characters, it gets completely unreadable, but it works nonetheless.</li> </ul> | Input: `"Hello my friend, how are you?"` <br> Output: `"Hello", "Ġmy", Ġfriend", ",", "Ġhow", "Ġare", "Ġyou", "?"`  |
